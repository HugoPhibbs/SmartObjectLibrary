...
from src.site.core.cloud.aws import get_aws_session, get_session_creds

load_dotenv()

# Script to load the OpenSearch client
__all__ = ["get_os_client"]
...
def get_os_client(stage=None, auth_type=None):
    ...
    if stage == "dev":
        ...
        session = get_aws_session(use_default=(auth_type == "lambda-role"), region=region)
        creds = get_session_creds(session)
